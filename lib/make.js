...
function makeAggregatePipeline(ast, options = {}) {
    ...
    if (ast.where) {
        if (whereContainsOtherTable(ast.where)) {
            const rightAst = ast.where.right.value[0];
            const subPl = makeAggregatePipeline(rightAst, options);
            pipeline = pipeline.concat(subPl);
            const localTableName = ast.from[0].table;
            pipeline.push({
                $lookup: {
                    from: localTableName,
                    localField: rightAst.columns[0].expr.column,
                    foreignField: ast.where.left.column,
                    as: localTableName,
                },
            });
            pipeline.push({
...
function makeMongoAggregate(sqlOrAST, options = {unwindJoins: false}) {
    ...
    const getTables = (subAst, first) => {
        ...
        if (whereContainsOtherTable(subAst.where)) {
            const subTables = getTables(subAst.where.right.value[0]);
