...
function makeAggregatePipeline(ast, options = {}) {
    ...
    if (ast.where) {
        if (whereContainsOtherTable(ast.where)) {
            const queries = getWhereAstQueries(ast.where);
            queries.forEach((astWithQuery) => {
                const subPl = makeAggregatePipeline(astWithQuery, options);
                pipeline = pipeline.concat(subPl);
                const localTableName = astWithQuery.from[0].table;
                const foreignField = astWithQuery.where.left.column || astWithQuery.where.right.column;
                pipeline.push({
                    $lookup: {
                        from: localTableName,
