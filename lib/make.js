...
function getWhereAstQueries(where) {
    if (where.right.value) {
        return where.right.value.filter(notAStandardValue);
    }
    if (where.left.value) {
        return where.left.value.filter(notAStandardValue);
    }
...
function getWhereStandardQueries(where) {
    // if (where.right.value) {
    //     return where.right.value.filter(isStandardValue);
    // }
    // if (where.left.value) {
    //     return where.left.value.filter(isStandardValue);
    // }
    if (where.left.left || where.right.left) {
        let queries = [];
        if (where.left.left) {
...
function notAnAstQuery(expr) {
    return !expr.value || !Array.isArray(expr.value) || (Array.isArray(expr.value) && expr.value.every(isStandardValue));
}
/**
 *
 * @param {*} expr
...
function makeAggregatePipeline(ast, options = {}) {
    ...
    if (ast.where) {
        if (whereContainsOtherTable(ast.where)) {
            const astQueries = getWhereAstQueries(ast.where);
            const localTableName = ast.from[0].table;
            const foreignField = ast.where.left.column || ast.where.left.left.column;
            astQueries.forEach((astWithQuery) => {
                const subPl = makeAggregatePipeline(astWithQuery, options);
                pipeline = pipeline.concat(subPl);

                pipeline.push({
                    $lookup: {
                        from: localTableName,
                        localField: astWithQuery.columns[0].expr.column,
                        foreignField,
                        as: localTableName,
                    },
                });
...
function makeMongoAggregate(sqlOrAST, options = {unwindJoins: false}) {
    ...
    const getTables = (subAst, first) => {
        ...
        if (whereContainsOtherTable(subAst.where)) {
            ...
            const subTables = queries.flatMap(getTables);
