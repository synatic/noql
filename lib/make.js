...
function getWhereAstQueries(where) {
    if (where.right.value) {
        return [{column: where.left.column, ast: where.right.value.filter(notAStandardValue)}];
    }
    if (where.left.value) {
        return [{column: where.right.column, ast: where.left.value.filter(notAStandardValue)}];
    }
