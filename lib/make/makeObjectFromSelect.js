...
function makeObjectFromSelect(ast) {
    ...
    columns.forEach((column) => {
        if (column.expr.type === 'column_ref') {
            ...
            toParse[`${column.as}`] = getParsedValueFromBinaryExpressionModule.getParsedValueFromBinaryExpression(column.expr);
        } else if (column.expr.type === 'case' && column.as) {
            toParse[`${column.as}`] = makeCaseConditionModule.makeCaseCondition(column.expr);
        } else if (column.expr.type === 'cast' && column.as) {
            toParse[`${column.as}`] = makeCastPartModule.makeCastPart(column.expr);
        } else if (column.expr.type === 'select' && column.as && column.expr.from) {
            toParse[`${column.as}`] = makeArraySubSelectPartModule.makeArraySubSelectPart(column.expr);
        } else if (column.expr.type === 'select' && column.as && !column.expr.from) {
