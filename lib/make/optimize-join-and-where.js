...
/**
 *
 * @param {import('../types').TableDefinition[]} from
 * @param {number} [joinIndex]
 * @returns {{sourceName: string, destinationName: string}}
 */
function getJoinSourceAndDestination(from, joinIndex = 1) {
    if (!from || from.length < 2) {
        throwJoinOptimizationError(
            `Join optimization requires at least two FROM entries (a base table and a join), but found ${from?.length ?? 0}.`,
            {fromCount: from?.length ?? 0}
        );
    }
    const sourceFrom = from[joinIndex - 1];
    const destinationFrom = from[joinIndex];
    if (!sourceFrom || !destinationFrom) {
        const fromDescriptions = from.map((entry, index) => {
            const name = entry?.as || entry?.table || '(unknown)';
            return `FROM[${index}]: ${name}`;
        });
        throwJoinOptimizationError(
            `Join optimization could not resolve the source and destination tables for join index ${joinIndex}. ` +
                `Expected FROM[${joinIndex - 1}] (source) and FROM[${joinIndex}] (destination) to be defined. ` +
                `Found: ${fromDescriptions.join(', ')}.`,
            {
                joinIndex,
                fromDescriptions,
            }
    ...
    // Aliases may carry join hints (e.g. `lob|first|optimize`), but field
    // references in the WHERE clause use the bare alias (e.g. `lob.field`).
    // Strip the hints so source/destination predicate detection works.
    const sourceName = cleanAlias(sourceFrom.as) || sourceFrom.table;
    const destinationName = cleanAlias(destinationFrom.as) || destinationFrom.table;
    if (!sourceName || !destinationName) {
        throwJoinOptimizationError(
            `Join optimization requires both the source and joined table to have a table name or alias. ` +
                `Source (FROM[${joinIndex - 1}]): ${sourceName || '(missing)'}, ` +
                `Destination (FROM[${joinIndex}]): ${destinationName || '(missing)'}.`,
            {joinIndex, sourceName, destinationName}
        );
    }
 ...
 */
function throwJoinOptimizationError(message, details = {}) {
    /** @type {Error & {joinOptimizationDetails?: Record<string, unknown>}} */
    const error = new Error(message);
    error.name = 'JoinOptimizationError';
    error.joinOptimizationDetails = details;
    throw error;
}
 ...
 * @returns {Error}
 */
function enrichJoinOptimizationError(err, context, ast) {
    /** @type {Error & {joinOptimizationDetails?: Record<string, unknown>}} */
    const joinErr = err;
    if (
        joinErr.name === 'JoinOptimizationError' ||
        joinErr.joinOptimizationDetails
    ) {
        const statement =
            context.cleanedStatement || context.rawStatement || '';
        const fromSummary = (ast?.from || []).map((entry, index) => {
            const name = entry?.as || entry?.table || '(unknown)';
            const joinType =
                entry && 'join' in entry && entry.join
                    ? ` (${entry.join})`
                    : '';
            return `  FROM[${index}]: ${name}${joinType}`;
        });
        const parts = [
            `Join optimization failed: ${err.message}`,
            fromSummary.length
                ? `Query FROM clause:\n${fromSummary.join('\n')}`
                : 'Query FROM clause: (empty)',
        ];
        if (statement) {
            const preview =
                statement.length > 500
                    ...
                    : statement;
            parts.push(`NOQL: ${preview}`);
        }
        /** @type {Error & {joinOptimizationDetails?: Record<string, unknown>}} */
        const enriched = new Error(parts.join('\n'));
        enriched.name = joinErr.name;
        enriched.joinOptimizationDetails = joinErr.joinOptimizationDetails;
        enriched.cause = joinErr;
        return enriched;
    }
    return err;
}
 ...
 * @returns {void}
 */
function optimizeJoinAndWhere(pipeline, pipeLineJoin, wherePiece, context, ast) {
    ...
    if (
        ...
        try {
            ...
            console.error(enrichJoinOptimizationError(err, context, ast));
...
function recursivelyOptimize(
    ...
    const {sourceName, destinationName} = getJoinSourceAndDestination(
        ast.from
    );
    /** @type {MatchType} */
    ...
    if (miscMatches.length > 0) {
        if (matchType === '$or' && parentMatchType === '$and') {
            mergeProcessResults(
                {
                    wasOptimized: false,
                    leftOverMatches: [{$or: miscMatches}],
...
/**
 * Strips join hints (e.g. `|first|optimize`) from a table alias, returning the
 * bare alias that field references actually use.
 * @param {string} alias
 * @returns {string}
 */
function cleanAlias(alias) {
    return typeof alias === 'string' ? alias.split('|')[0] : alias;
}
        ...
        .map((d) => {
            ...
            if (d.$and) {
                ...
                const hasSource = andConditions.some((condition) => {
                    ...
                    return (
                        keys.length > 0 && keys[0].startsWith(`${sourceName}.`)
                    );
                });
