...
function recursivelyOptimise(
    ...
    const andResults = processMatchesForAnds(
        matches,
        lookup,
        pipeline,
        context
    );
    mergeProcessResults(andResults, returnResult);
    const orResults = processMatchesForOrs(matches, lookup, pipeline, context);
    mergeProcessResults(orResults, returnResult);
...
function processSourceMatches(
    ...
    function findTarget() {
        ...
        const matchingSteps = currentResult.pipelineStagesAdded.filter(
            (stage) => !!get(stage, `$match.${matchType}`)
        );
        if (matchingSteps.length > 0) {
            if (matchingSteps.length === 1) {
                return matchingSteps[0].$match[matchType];
            } else {
    ...
    function findTarget() {
        const addedPipelineStageTarget =
            currentResult.lookupPipelineStagesAdded.filter(
                (stage) => !!get(stage, `$match.$expr.${matchType}`)
            );
        if (addedPipelineStageTarget.length > 0) {
            if (addedPipelineStageTarget.length === 1) {
                return addedPipelineStageTarget[0].$match.$expr[matchType];
            }
        ...
        if (pipelineTarget.length > 0) {
            if (pipelineTarget.length === 1) {
                return pipelineTarget[0].$match.$expr[matchType];
            }
