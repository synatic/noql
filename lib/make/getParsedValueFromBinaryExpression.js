...
function getParsedValueFromBinaryExpression(
    ...
    if (expressionPart.type === 'column_ref') {
        ...
        const nestedPath = expressionPart.schema
            ? `${expressionPart.table}.${expressionPart.column}`
            : expressionPart.column;
        const alias = expressionPart.schema || expressionPart.table;
        return `$${alias ? alias + '.' : ''}${nestedPath}`;
    }
