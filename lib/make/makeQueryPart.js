...
function makeQueryPart(
    ...
    const getColumnNameOrVal = (queryPart) => {
        ...
        // `table` there is part of the real field path, not an alias.
        const nestedPath = queryPartToUse.schema
            ? `${queryPartToUse.table}.${queryPartToUse.column}`
            : queryPartToUse.column;
        const table = queryPartToUse.schema || queryPartToUse.table || tableAlias;
    ...
    const makeOperator = (op) => {
        ...
        if (
            ...
            let leftToUse =
                $check.string(left) && !leftIsValueType && !left.startsWith('$')
                    ? `$${left}`
                    : left;
