...
function makeJoinForPipeline(ast) {
    ...
    const makeJoinPart = (join, previousJoin) => {
        ...
        if (join.table && join.on && join.on.type === 'binary_expr' && join.on.operator === '=') {
            ...
            const joinQuery = makeFilterConditionModule.makeFilterCondition(join.on, false, true);
            const inputVars = {};
            const replacePaths = [];
            ...
            if (join.expr && join.expr.ast) {
                lookupPipeline = makeAggregatePipelineModule.makeAggregatePipeline(join.expr.ast);
