...
/**
 * For a simple equality join (binary_expr with two column_refs), resolves which FROM
 * in ast.from[0..joinIndex-1] the "local" side of the join refers to (the side that is
 * not the join target). Used to compute the correct localField when a join references
 * the base table or an earlier join rather than the immediately previous one.
 * @param {import('../types').TableDefinition} join - the join at ast.from[joinIndex]
 * @param {import('../types').TableDefinition[]} fromList - ast.from
 * @param {number} joinIndex - index of this join in fromList
 * @returns {import('../types').TableDefinition|null} the FROM the local side refers to, or null to use fallback (previous FROM)
 */
function resolveLocalFromForJoin(join, fromList, joinIndex) {
    if (
        !join.on ||
    ...
    const toTable = (join.table || '').split('|')[0];
    const toAs = (join.as || '').split('|')[0];
    const leftRef = join.on.left;
    const rightRef = join.on.right;

    const leftTable = (leftRef.table || '').split('|')[0];
    const rightTable = (rightRef.table || '').split('|')[0];

    const rightIsTarget = rightTable === toAs || rightTable === toTable;
    const leftIsTarget = leftTable === toAs || leftTable === toTable;
    ...
    const localTable = (localPart.table || '').split('|')[0];
    const precedingFroms = fromList.slice(0, joinIndex);

    if (localTable) {
        const match = precedingFroms.find((f) => {
            const fAs = (f.as || '').split('|')[0];
            const fTable = (f.table || '').split('|')[0];
            return fAs === localTable || fTable === localTable;
        });
        return match || null;
    }

    return precedingFroms[0] || null;
}
...
function makeJoinForPipeline(ast, context) {
    ...
    for (let i = 1; i < ast.from.length; i++) {
        ...
        const fallbackPrevious = ast.from[i - 1];
        const resolvedLocalFrom = resolveLocalFromForJoin(join, ast.from, i);
        const previousJoin = resolvedLocalFrom || fallbackPrevious;

        makeJoinPart(join, previousJoin, aliases, pipeline, context, ast);
    }
...
function makeJoinPart(join, previousJoin, aliases, pipeline, context, ast) {
    ...
    if (
        ...
        return tableJoin(
            join.as ? {...join, as: join.as.split('|')[0]} : join,
            previousJoin.as
                ? {...previousJoin, as: previousJoin.as.split('|')[0]}
                : previousJoin,
            pipeline,
            toTable,
            toAs,
