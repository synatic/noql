...
function buildConcatForObject(schema, from) {
    ...
    for (const propName in schema.properties) {
        ...
        if (keys.length === 1) {
            const subPipeline = typeMap[keys[0]];
            if (Array.isArray(subPipeline)) {
                concatItems.push(...subPipeline);
            } else {
                concatItems.push(subPipeline);
            }
        } else {
            ...
            for (const key of keys) {
                ...
                switch (key) {
                    case 'object':
                        conditions.push(
                            makeCondition(fullPropName, 'object', subPipeline)
                        );
                        break;
                    case 'array':
                        conditions.push(
                            makeCondition(fullPropName, 'array', subPipeline)
                        );
                        break;
                    case 'boolean':
                        conditions.push(
                            makeCondition(fullPropName, 'bool', subPipeline)
                        );
                        break;
                    case 'integer':
                    case 'number':
                    ...
                    case 'string':
                        conditions.push(
                            makeCondition(fullPropName, 'string', subPipeline)
                        );
                        break;
                    case 'null':
                        break;
            ...
            const reduced = conditions.reduce((previous, current, index) => {
                ...
                return previous;
            }, makeCondition(fullPropName, 'missing', ''));
 ...
 * @param {*}subPipeline
 */
function makeCondition(propName, mongoType, subPipeline) {
    // todo RK support for multiple mongoTypes.
    if (Array.isArray(subPipeline)) {
