    ...
    for (const propName in schema.properties) {
        ...
        for (const type of propertyTypes) {
            switch (type) {
                case 'object':
                    {
                        const res = buildConcatForObject(propValue, nestedFrom);
                        if (res) {
                ...
                case 'array':
                    ...
                    // if (res) {
                    //     concatItems.push(`"${propName}":`);
                    //     concatItems.push(res);
                    // }
        ...
        if (keys.length === 1) {
            const itemToPush = typeMap[keys[0]];
            if (Array.isArray(itemToPush)) {
                concatItems.push(...itemToPush);
            } else {
                concatItems.push(itemToPush);
            }
        } else {
        ...
        //             if (hasArrays && hasObjects) {
        //                 throw new Error(
        //                     `Schema can be both an object and an array, not yet supported ${JSON.stringify(
        //                         notNullTypes
        //                     )}`
        //                 );
        //             } else if (hasArrays) {
