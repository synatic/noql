    ...
    if (ast.columns && !isSelectAll(ast.columns) && ast.columns.length > 0) {
        ...
        columns.forEach((column) => {
            if (column.expr.type === 'column_ref') {
                mapIn[column.as || column.expr.column] = `$$this.${column.expr.table ? column.expr.table + '.' : ''}${column.expr.column}`;
            } else if (column.expr.type === 'function' && column.as) {
                mapIn[column.as] = makeProjectionExpressionPartModule.makeProjectionExpressionPart(column.expr, depth + 1);
            } else if (column.expr.type === 'aggr_func' && column.as) {
                mapIn[column.as] = makeProjectionExpressionPartModule.makeProjectionExpressionPart(column.expr, depth + 1);
            } else if (column.expr.type === 'binary_expr' && column.as) {
                mapIn[column.as] = getParsedValueFromBinaryExpressionModule.getParsedValueFromBinaryExpression(column.expr);
            } else if (column.expr.type === 'case' && column.as) {
                mapIn[column.as] = makeCaseConditionModule.makeCaseCondition(column.expr);
            } else if (column.expr.type === 'select' && column.as && column.expr.from) {
                mapIn[column.as] = makeArraySubSelectPart(column.expr, depth + 1);
            } else if (column.expr.type === 'select' && column.as && !column.expr.from) {
                mapIn[column.as] = makeObjectFromSelectModule.makeObjectFromSelect(column.expr);
            } else if (column.expr.type && column.as) {
                mapIn[column.as] = {$literal: column.expr.value};
            } else if (!column.as) {
                throw new Error(`Require as for array subselect calculation:${column.expr.name}`);
            } else {
