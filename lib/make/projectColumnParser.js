 ...
 * @typedef {import('../types').AST} AstLike
...
function projectColumnParser(column, result) {
    ...
    if (column.expr.type === 'function' && column.as && column.expr.name && column.expr.name.toLowerCase() === 'unwind') {
        ...
        result.parsedProject.$project[column.as] = makeProjectionExpressionPartModule.makeProjectionExpressionPart(
            column.expr.args.value[0]
        );
        result.unwind.push({
            $unwind: `$${column.as}`,
    ...
    if (column.expr.type === 'function' && column.as) {
        ...
        const parsedExpr = makeProjectionExpressionPartModule.makeProjectionExpressionPart(column.expr);
        if (column.as && column.as.toUpperCase() === '$$ROOT') {
    ...
    if (column.expr.type === 'aggr_func') {
        result.parsedProject.$project[column.as] = makeProjectionExpressionPartModule.makeProjectionExpressionPart(column.expr);
        return;
    }
    if (column.expr.type === 'binary_expr' && column.as) {
        result.parsedProject.$project[column.as] = getParsedValueFromBinaryExpressionModule.getParsedValueFromBinaryExpression(column.expr);
        return;
    }
    if (column.expr.type === 'case' && column.as) {
        result.parsedProject.$project[column.as] = makeCaseConditionModule.makeCaseCondition(column.expr);
        return;
    }
    if (column.expr.type === 'cast' && column.as) {
        result.parsedProject.$project[column.as] = makeCastPartModule.makeCastPart(column.expr);
        return;
    }
    if (column.expr.type === 'select' && column.as && column.expr.from) {
        result.parsedProject.$project[column.as] = makeArraySubSelectPartModule.makeArraySubSelectPart(column.expr);
        return;
    }
    if (column.expr.type === 'select' && column.as && !column.expr.from) {
        result.parsedProject.$project[column.as] = makeObjectFromSelectModule.makeObjectFromSelect(column.expr);
        return;
    }
