...
function projectColumnParser(column, result, context, tableAlias = '') {
    if (column.expr.type === 'column_ref') {
        // `schema` holds the true table alias for a nested (3+ level) dotted
        // path, e.g. `s.a.b` parses as {schema:'s', table:'a', column:'b'}.
        const columnValue = column.expr.schema
            ? `${column.expr.table}.${column.expr.column}`
            : column.expr.column;
        const columnTable = column.expr.schema || column.expr.table || tableAlias;
        ...
        const expression =
            columnTable === columnValue.split('.')[0]
                ? `$${columnValue}`
                : `$${columnTable ? columnTable + '.' : ''}${columnValue}`;
    ...
    if (column.expr.type === 'function' && !column.as) {
        ...
        if (parsedExpr.$set) {
            result.set = {$set: parsedExpr.$set};
            applied++;
        }
        if (parsedExpr.unsetAfterReplaceOrSet) {
            result.unsetAfterReplaceOrSet = parsedExpr.unsetAfterReplaceOrSet;
            applied++;
        }
