...
const {normalizeFieldReference} = require('./buildFieldReference');
...
function makeFilterCondition(
    ...
    if (queryPart.type === 'column_ref') {
        ...
        // Array subselects: normalize legacy `$this`/`$$this` column hacks.
        // Join correlation vars use prefixLeft/prefixRight which already add a
        // single `$`; the historical second `$` (→ `$$orders.item`) must be
        // preserved and must not go through normalizeFieldReference.
        if (includeThis) {
            return normalizeFieldReference(
                `${prefix}${queryPart.column}`,
                true
            );
        }
