...
function makeMongoAggregate(sqlOrAST, options = {unwindJoins: false}) {
    ...
    return {
        pipeline: makeAggregatePipelineModule.makeAggregatePipeline(ast, options),
...
function makeMongoQuery(sqlOrAST, options = {}) {
    ...
    if (ast.where) {
        parsedQuery.query = makeQueryPartModule.makeQueryPart(ast.where, true, [], false);
    }
