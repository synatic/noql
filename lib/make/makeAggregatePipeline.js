...
function makeAggregatePipeline(ast, context = {}) {
    ...
    if (ast._next && ast.set_op && ast.set_op === 'intersect') {
        ...
        if (!intersectionCollectionName) {
            throw new Error('No collection to INTERSECT with');
        }
        pipeline.push({
            $unionWith: {
        ...
        const _idField = firstQueryFields.reduce(
            (obj, {name, value}, index) => {
                const {name: otherName, value: otherValue} =
                    secondQueryFields[index];
                if (name === otherName) {
                    obj[name] = value;
                } else {
                    obj[name] = {
        ...
        pipeline.push({$group: {_id: _idField, count: {$sum: 1}}});
        pipeline.push({
            $match: {count: {$gt: 1}},
        });
        pipeline.push({$replaceRoot: {newRoot: '$_id'}});
