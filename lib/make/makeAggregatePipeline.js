...
function makeAggregatePipeline(ast, options = {}) {
    ...
    if (ast.where) {
        if (whereContainsOtherTable(ast.where)) {
            ...
            standardQueries.forEach((sq) => {
                pipeline.push({
                    $match: makeQueryPartModule.makeQueryPart(sq, false, [], false),
                });
            });
        } else {
            wherePiece = {
                $match: makeQueryPartModule.makeQueryPart(ast.where, false, [], false),
            };
        }
    }
    ...
    if (ast.groupby) {
        ...
        if (ast.having) {
            pipeline.push({$match: makeQueryPartModule.makeQueryPart(ast.having)});
        }
