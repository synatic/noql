...
function makeProjectionExpressionPart(
    ...
    if (!expr.name && !expr.operator) {
        return makeArg(expr, depth, context, includeThis);
    }
    ...
    if ((expr.args && expr.args.value) || fn.doesNotNeedArgs) {
        ...
        return fn.parse(
            args.map((a) => makeArg(a, depth, context, includeThis)),
            depth,
            forceLiteralParse
        );
        ...
        return fn.parse(
            makeArg(expr.args.expr, depth, context, includeThis),
            depth,
            forceLiteralParse
        );
    } else {
 ...
 * @param {boolean} [includeThis] - prefix fields with $$this for array subselects
 */
function makeArg(expr, depth, context, includeThis = false) {
    if (expr.type === 'function' || expr.type === 'aggr_func') {
        return makeProjectionExpressionPart(
            expr,
            context,
            depth,
            false,
    ...
    if (expr.type === 'column_ref') {
        const path = `${expr.table ? expr.table + '.' : ''}${expr.column}`;
        return includeThis ? `$$this.${path}` : `$${path}`;
    }
    ...
    if (expr.type === 'case') {
        return makeCaseConditionModule.makeCaseCondition(
            expr,
            context,
            includeThis
        );
    ...
    if (!expr.type && expr.ast) {
        return makeArg(expr.ast, depth, context, includeThis);
    }
