...
function canQuery(sqlOrAST, options = {isArray: false}) {
    ...
    const checkWhereContainsExpression = (expr) => {
        if (!expr) {
            return false;
        }
        if (expr.type === 'binary_expr') {
            return checkWhereContainsExpression(expr.left) || checkWhereContainsExpression(expr.right);
        }
    ...
    return !(
        (
            ...
            checkWhereContainsExpression(ast.where)
        )
