...
function canQuery(sqlOrAST, options = {isArray: false}) {
    ...
    const asColumns = isSelectAll ? [] : ast.columns.map((c) => c.as).filter((c) => !!c);
    ...
    const containsAllowedAggregateFunctions = !isSelectAll && ast.columns.findIndex(checkIfContainsAllowedAggregateFunctions) > -1;
    const hasAsRoot = !isSelectAll && !options.isArray && ast.columns.findIndex((c) => c.as === '$$ROOT') > -1;
    const containsAllowedFunctions = !isSelectAll && ast.columns.findIndex(checkIfContainsAllowedFunctions) > -1;
    const containsColumnRefAsterix =
        !isSelectAll && ast.columns.findIndex((c) => c.expr.type === 'column_ref' && c.expr.column === '*') > -1;
