...
function canQuery(sqlOrAST, options = {isArray: false}) {
    ...
    const asColumns = isSelectAll ? [] : columns.map((c) => c.as).filter((c) => !!c);
    ...
    const containsAllowedAggregateFunctions = !isSelectAll && columns.findIndex(checkIfContainsAllowedAggregateFunctions) > -1;
    const hasAsRoot = !isSelectAll && !options.isArray && columns.findIndex((c) => c.as === '$$ROOT') > -1;
    const containsAllowedFunctions = !isSelectAll && columns.findIndex(checkIfContainsAllowedFunctions) > -1;
    const containsColumnRefAsterix = !isSelectAll && columns.findIndex((c) => c.expr.type === 'column_ref' && c.expr.column === '*') > -1;
    const whereContainsOtherTable = checkWhereContainsOtherTable(ast.where);
    const asColumnsUsedInWhere = asColumns.length > 0 && checkAsUsedInWhere(ast.where);
...
function checkIfContainsAllowedFunctions(column) {
    return (
        ...
        !_allowableFunctions.functionMappings.find(
            (f) => f.name === column.expr.name.toLowerCase() && (!f.type || f.type === column.expr.type) && f.allowQuery
        )
    );
}
...
function checkWhereContainsOtherTable(expr) {
    ...
    if (expr.type === 'binary_expr') {
        return checkWhereContainsOtherTable(expr.left) || checkWhereContainsOtherTable(expr.right);
    }
    if (expr.type === 'expr_list') {
        return !expr.value.every((val) => ['number', 'string', 'single_quote_string'].includes(val.type));
    }
    return false;
}

module.exports = {canQuery, whereContainsOtherTable: checkWhereContainsOtherTable};
