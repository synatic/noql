...
const _patternsToFix = [
    {
        ...
        fixerFn: (stages) => {
            ...
            if (projectIsRoot(stages[0]) && !projectIsRoot(stages[1])) {
                ...
                // When inlining, any field that passed through the root alias (e.g. "c": "$c")
                // must be set to "$$ROOT" so the alias is preserved; otherwise "c" would be
                // undefined and later stages reading $c.* would get null.
                const rootRef = '$' + testProject1Key;
                for (const key of Object.keys(newProject)) {
                    if (newProject[key] === rootRef) {
                        newProject[key] = '$$ROOT';
                    }
                }
...
function _matchPieceIsSimple(objVal, prefix) {
    // BSON values (ObjectId, Long, Decimal128, Binary, etc.) expose own
    // properties like `buffer`, so check-types treats them as objects. Treat
    // them as literal match values so late WHERE stages can still be hoisted.
    if (_isBsonLiteralValue(objVal)) {
        return true;
    }
...
/**
 * Returns true for BSON typed values that are safe match literals.
 * @param {any} value
 * @returns {boolean}
 */
function _isBsonLiteralValue(value) {
...
/**
 * Checks if a value is a Mongo field reference.
 * @param {any} value
 * @returns {boolean}
 */
function _isFieldReference(value) {
    return (
        $check.string(value) && value.startsWith('$') && !value.startsWith('$$')
    );
}

/**
 * Checks if a value can be used in a plain $match (without $expr).
 * Only BSON primitives are safe; objects/arrays may contain aggregation operators.
 * @param {any} value
 * @returns {boolean}
 */
function _isHoistableLookupLiteralValue(value) {
    if (value === null || $check.boolean(value) || $check.number(value)) {
        return true;
    }
    if ($check.date(value)) {
        return true;
...
/**
 * Gets a flat list of predicates from an expr object.
 * @param {object} expr
 * @returns {Array<object>}
 */
function _getExprPredicates(expr) {
    if (!expr) {
        return [];
    }
    if ($check.array(expr.$and)) {
        return expr.$and.flatMap((predicate) =>
            _getExprPredicates(predicate)
        );
    }
    return [expr];
}
...
/**
 * Builds an expr object from predicates.
 * @param {Array<object>} predicates
 * @returns {object|null}
 */
function _buildExprFromPredicates(predicates) {
...
/**
 * Extracts field/literal equality details from a predicate if safe.
 * @param {object} predicate
 * @returns {{field: string, literalValue: any}|null}
 */
function _extractLookupLiteralEqPredicate(predicate) {
    if (
        !predicate ||
        !predicate.$eq ||
        !$check.array(predicate.$eq) ||
        predicate.$eq.length !== 2
    ) {
        return null;
    }

    const left = predicate.$eq[0];
    const right = predicate.$eq[1];

    if (_isFieldReference(left) && _isHoistableLookupLiteralValue(right)) {
        return {
            field: left.substring(1),
            literalValue: right,
        };
    }
    if (_isFieldReference(right) && _isHoistableLookupLiteralValue(left)) {
        return {
            field: right.substring(1),
            literalValue: left,
        };
    }
    return null;
...
/**
 * Splits a lookup $match.$expr stage into direct match and expr stages when possible.
 * @param {object} stage
 * @returns {Array<object>}
 */
function _transformLookupExprMatchStage(stage) {
    if (!stage || !stage.$match || !stage.$match.$expr) {
        return [stage];
    }

    const predicates = _getExprPredicates(stage.$match.$expr);
    if (predicates.length === 0) {
        return [stage];
    }

    const directMatch = {};
    const nonHoistedPredicates = [];
    let hasHoistedPredicate = false;

    for (const predicate of predicates) {
        const lookupPredicate = _extractLookupLiteralEqPredicate(predicate);
        if (!lookupPredicate) {
            nonHoistedPredicates.push(predicate);
            continue;
        }

        hasHoistedPredicate = true;
        if (!(lookupPredicate.field in directMatch)) {
            directMatch[lookupPredicate.field] = lookupPredicate.literalValue;
        } else if (
            !lodash.isEqual(
                directMatch[lookupPredicate.field],
                lookupPredicate.literalValue
            )
        ) {
            nonHoistedPredicates.push(predicate);
        }
    ...
    if (!hasHoistedPredicate) {
        return [stage];
    }

    const transformedStages = [{$match: directMatch}];
    const rebuiltExpr = _buildExprFromPredicates(nonHoistedPredicates);
    if (rebuiltExpr) {
...
/**
 * Hoists literal equality predicates out of $expr inside a lookup pipeline.
 * @param {Array<object>} lookupPipeline
 * @returns {Array<object>}
 */
function _hoistLookupExprLiteralPredicates(lookupPipeline) {
    const transformedPipeline = [];
    for (const stage of lookupPipeline) {
        const transformedStages = _transformLookupExprMatchStage(stage);
        for (const transformedStage of transformedStages) {
            transformedPipeline.push(transformedStage);
        }
    }
...
/**
 * Builds a map of alias -> $path for simple 1:1 rename $project stages.
 * @param {object} projectStage
 * @returns {Record<string, string>|null}
 */
function _buildSimpleRenameMap(projectStage) {
    if (!projectIsSimple(projectStage)) {
        return null;
    }
    ...
    for (const alias of Object.keys(project)) {
        const path = project[alias];
        if (
            !$check.string(path) ||
            !path.startsWith('$') ||
            path.startsWith('$$')
...
/**
 * Rewrites a single $field reference using a simple rename map.
 * @param {string} fieldRef
 * @param {Record<string, string>} renameMap
 * @returns {string}
 */
function _rewriteFieldReferenceWithRenameMap(fieldRef, renameMap) {
    if (!_isFieldReference(fieldRef)) {
        return fieldRef;
    }

    const fieldPath = fieldRef.substring(1);
    const dotIndex = fieldPath.indexOf('.');
    const rootField = dotIndex === -1 ? fieldPath : fieldPath.substring(0, dotIndex);

    if (!(rootField in renameMap)) {
        return fieldRef;
    }

    const renamedRoot = renameMap[rootField].substring(1);
    if (dotIndex === -1) {
        return '$' + renamedRoot;
    }

    return '$' + renamedRoot + fieldPath.substring(dotIndex);
}

/**
 * Rewrites $match field references and keys using a simple rename map.
 * @param {object} match
 * @param {Record<string, string>} renameMap
 * @returns {object}
 */
function _rewriteMatchWithRenameMap(match, renameMap) {
    let result = $copy(match);
    const directKeys = Object.keys(result).filter((key) => {
        return !key.startsWith('$');
    });

    for (const key of directKeys) {
        const rootKey = key.split('.')[0];
        if (rootKey in renameMap) {
            const renamedRoot = renameMap[rootKey].substring(1);
            const suffix = key.includes('.') ? key.substring(rootKey.length) : '';
            const renamedKey = renamedRoot + suffix;
            if (renamedKey !== key) {
                result[renamedKey] = result[key];
                delete result[key];
            }
        }
    }

    result = $json.changeValue(result, (value) => {
        if (_isFieldReference(value)) {
            return _rewriteFieldReferenceWithRenameMap(value, renameMap);
        }
        return value;
    });
...
/**
 * Checks whether later pipeline stages still reference projected aliases.
 * @param {Record<string, string>} renameMap
 * @param {Array<object>} stages
 * @returns {boolean}
 */
function _remainingStagesNeedProjectAliases(renameMap, stages) {
    if (!stages || stages.length === 0) {
        return false;
    }

    const serialized = JSON.stringify(stages);
    for (const alias of Object.keys(renameMap)) {
        if (
...
/**
 * Moves lookup $match before simple 1:1 rename $project stages so indexes can be used.
 * @param {Array<object>} lookupPipeline
 * @returns {Array<object>}
 */
function _hoistLookupProjectBeforeMatch(lookupPipeline) {
    const transformedPipeline = [];
    let index = 0;

    while (index < lookupPipeline.length) {
        const projectStage = lookupPipeline[index];
        const matchStage = lookupPipeline[index + 1];

        if (projectStage.$project && matchStage && matchStage.$match) {
            const renameMap = _buildSimpleRenameMap(projectStage);
            if (renameMap) {
                const rewrittenMatch = _rewriteMatchWithRenameMap(
                    matchStage.$match,
                    renameMap
                );
                const remainingStages = lookupPipeline.slice(index + 2);
                const projectStillNeeded = _remainingStagesNeedProjectAliases(
                    renameMap,
                    remainingStages
                );

                transformedPipeline.push({$match: rewrittenMatch});
                if (projectStillNeeded) {
                    transformedPipeline.push(projectStage);
                }
                index += 2;
                continue;
            }
    ...
    while (iteration > 0 && lastHash !== $hash(newAggregate)) {
        ...
        for (const pipelineStage of newAggregate) {
            if (
                ...
                pipelineStage.$lookup.pipeline =
                    _hoistLookupProjectBeforeMatch(
                        pipelineStage.$lookup.pipeline
                    );
                pipelineStage.$lookup.pipeline =
                    _hoistLookupExprLiteralPredicates(
                        pipelineStage.$lookup.pipeline
                    );
            }
        }
