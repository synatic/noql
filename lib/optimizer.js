...
/**
 * Checks if a value should be treated as a literal in lookup expr eq checks.
 * @param {any} value
 * @returns {boolean}
 */
function _isLiteralLookupExprValue(value) {
    return !(
        $check.string(value) &&
...
function _extractLookupLiteralEqPredicate(predicate) {
    ...
    if (_isFieldReference(left) && _isLiteralLookupExprValue(right)) {
        return {
            field: left.substring(1),
            literalValue: right,
        };
    }
    if (_isFieldReference(right) && _isLiteralLookupExprValue(left)) {
        return {
