...
//todo restructure so public members are at top of class, private at bottom
//todo make sure private members are prefixed with _
//todo remove lodash requirement - only using has function, though makes code quicker makes package larger
class SQLParser {

    /**parse sql to either a query or aggregate
     *
     * @param sql
     * @param type
     * @param options
    ...
    static parseSQL(sql, type = "query", options = {unwindJoins:false}) {
        if (!sql) {
    ...
    static _canQuery(ast) {
        return !(
            ...
            || (ast.columns !== "*" && ast.columns.filter(c => c.expr.type === "function" && !AllowableFunctions._list.find(f => f.name === c.expr.name)).length > 0));
    }
    ...
    static makeMongoQuery(ast,options={}) {
    ...
    static getParsedValueFromBinaryExpression(expressionPart) {
        let result;

        if (expressionPart.type === 'binary_expr') {
            result = SQLParser.binaryExpressionLogic(expressionPart)
    ...
    static binaryExpressionLogic(column) {
        let operator
        if (column.expr) {
        ...
        if (aggregateFunction) {
            if (_.has(column, 'expr.left') && _.has(column, 'expr.right')) {
                const leftPartValue = SQLParser.getParsedValueFromBinaryExpression(column.expr.left);
                const rightPartValue = SQLParser.getParsedValueFromBinaryExpression(column.expr.right);

                aggregateResult = aggregateFunction.parse(leftPartValue, rightPartValue);
            } else if (_.has(column, 'left') && _.has(column, 'right')) {
                const leftPartValue = SQLParser.getParsedValueFromBinaryExpression(column.left);
                const rightPartValue = SQLParser.getParsedValueFromBinaryExpression(column.right);
    ...
    static aggregationFunctionLogic(column) {
        let aggregateResult;

        const aggregateFunction = AllowableFunctions._list.find(f => f.name === column.expr.name.toLowerCase());
        if (aggregateFunction) {
            if (_.has(column, 'expr.args.expr.args')) {
                const returnValue = SQLParser.aggregationFunctionLogic(column.expr.args)
                aggregateResult = AllowableFunctions.parse(aggregateFunction, returnValue)
            }

            if (_.has(column, 'expr.args.expr.column')) {
                aggregateResult = aggregateFunction.parse(column.expr.args.expr.column);
            } else if (_.has(column, 'expr.args.expr.value')) {
                aggregateResult = aggregateFunction.parse(column.expr.args.expr.value);
            } else if (_.has(column, 'expr.args.value[0].column')) {
    ...
    //todo make accept string or ast
    static makeMongoAggregate(ast,options={}) {
        let parsedQuery = {
            pipeline: [],
            collections: []
        };
        ...
        if (ast.from.length) {
            ...
            ast.from.forEach(from => {
                if (from.join) {
                    lookup = SQLParser.joinFunctionLogic(from.on)
                }
        ...
        if (ast.columns && ast.columns !== "*" && ast.columns.length > 0) {
            ...
            ast.columns.forEach(column => {
                if (column.as || _.has(column, 'expr.name')) {
                    project[column.as] = 1;

                    let aggregateResult;

                    if (_.has(column, 'expr.name')) {
                        aggregateResult = SQLParser.aggregationFunctionLogic(column)
                    } else if (column.expr.type === 'binary_expr') {
                        aggregateResult = SQLParser.binaryExpressionLogic(column);
                    }

                    if (aggregateResult) {
                        if (_.has(ast, 'groupby.length')) {
