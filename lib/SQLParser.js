...
class SQLParser {
    ...
    static _makeProjectionFunctionPart(expr) {
        let fn = _allowableFunctions.find(f => f.name === expr.name.toLowerCase());
        if (!fn) {
    ...
    static binaryExpressionLogic(column) {
        ...
        const aggregateFunction = _allowableFunctions.find(f => f.name === operator.toLowerCase());
        let aggregateResult;
            ...
            ast.columns.forEach(column => {
                if (column.as) {
                    ...
                    if (_.has(column, 'expr.name')) {
                        aggregateFunction = _allowableFunctions.find(f => f.name === column.expr.name.toLowerCase());
                        if (aggregateFunction) {
                            if (_.has(column, 'expr.args.expr.column')) {
                                ...
                                aggregateResult = aggregateFunction.parse(column.expr.args.value[0].column);
                            }
                        }
                    ...
                    // if (aggregateFunction.inside === 'group') {
                    //     group[column.as] = aggregateResult;
                    // } else if (aggregateFunction.inside === 'project') {
                    ...
                    projection[column.expr.string || column.expr.column] = 1;
