...
class SQLParser {
    ...
    static _makeProjectionFunctionPart(expr, depth = 0) {
    ...
    static _useCast(expr) {
        let fn = AllowableFunctions._list.find(f => f.name.toLowerCase() === expr.type.toLowerCase() && (!f.type || f.type === expr.type));

        if (!fn) {
            throw new Error(`Function:${expr.type} not available`);
        }

        let input;
        if (expr.expr.column) {
            input = `$${expr.expr.column}`
        } else if (expr.expr.value) {
            input = expr.expr.value
        } else {
            throw new Error(`Function:${expr.type} not supported argument`);
        }

        let to = expr.target.dataType.toLowerCase();

        return fn.parse([input, to])
    }
    ...
    static _getParsedValueFromBinaryExpression(expressionPart) {
        ...
        if (expressionPart.type === 'binary_expr') {
            ...
            result = SQLParser._makeProjectionFunctionPart(expressionPart);
        }
        ...
        if (ast.columns && ast.columns !== "*" && ast.columns.length > 0) {
            ...
            ast.columns.forEach(v => {
                if (v.expr.type === "column_ref") {
                    ...
                    mapIn[v.as] = SQLParser._makeProjectionFunctionPart(v.expr, depth + 1);
    ...
    static makeMongoAggregate(sqlOrAST, options = { unwindJoins: false }) {
        ...
        if (ast.limit) {
            if (ast.limit.seperator && ast.limit.seperator === "offset" && ast.limit.value[1] && ast.limit.value[1].value) {
                parsedQuery.pipeline.push({ $limit: ast.limit.value[0].value });
                parsedQuery.pipeline.push({ $skip: ast.limit.value[1].value });
            } else if (ast.limit.value && ast.limit.value[0] && ast.limit.value[0].value) {
                parsedQuery.pipeline.push({ $limit: ast.limit.value[0].value });
            }
        }

        parsedQuery.collections = tableList.map(t => t.split('::')[2]);
