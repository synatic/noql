...
class SQLParser {
    ...
    static _makeBinaryExpresionPart(expr) {
        ...
        const exprFunction = _allowableFunctions.functionList.find((f) => f.name === operator.toLowerCase());
        let exprResult;
        if (exprFunction) {
    ...
    static _validateAST(parsedAST) {
        ...
        if ($check.array(ast.columns)) {
            ...
            for (let column of ast.columns) {
                ...
                if(column.expr&&(column.expr.type==='function'||column.expr.type==='aggr_func')&&column.expr.name.toLowerCase()==="unwind"){
                    continue;
                }
                if(column.expr&&column.expr.type==='function'&&!_allowableFunctions.functionList.find((f) => f.name === column.expr.name.toLowerCase() && (!f.type || f.type === column.expr.type))){
                    errors.push(`Function not found: ${column.expr.name}`);
                }
                if(column.expr&&column.expr.type==='aggr_func'&&!_allowableFunctions.functionList.find((f) => f.name === column.expr.name.toLowerCase() && (!f.type || f.type === column.expr.type))){
