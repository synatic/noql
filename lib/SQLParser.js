...
class SQLParser{
    ...
    static _makeQueryPart(queryPart,ignorePrefix) {
        ...
        if (queryPart.operator === "=") {
            return {$eq: [SQLParser._makeQueryPart(queryPart.left, ignorePrefix), SQLParser._makeQueryPart(queryPart.right, ignorePrefix)]}
        } else if (queryPart.operator === ">") {
            return {$gt: [SQLParser._makeQueryPart(queryPart.left, ignorePrefix), SQLParser._makeQueryPart(queryPart.right, ignorePrefix)]}
        } else if (queryPart.operator === "<") {
            return {$lt: [SQLParser._makeQueryPart(queryPart.left, ignorePrefix), SQLParser._makeQueryPart(queryPart.right, ignorePrefix)]}
        } else if (queryPart.operator === ">=") {
            return {$gte: [SQLParser._makeQueryPart(queryPart.left, ignorePrefix), SQLParser._makeQueryPart(queryPart.right, ignorePrefix)]}
        } else if (queryPart.operator === "<=") {
            return {$lte: [SQLParser._makeQueryPart(queryPart.left, ignorePrefix), SQLParser._makeQueryPart(queryPart.right, ignorePrefix)]}
        } else if (queryPart.operator === "!=") {
            return {$neq: [SQLParser._makeQueryPart(queryPart.left, ignorePrefix), SQLParser._makeQueryPart(queryPart.right, ignorePrefix)]}
        } else if (queryPart.operator === "AND") {
            return {$and: [SQLParser._makeQueryPart(queryPart.left, ignorePrefix), SQLParser._makeQueryPart(queryPart.right, ignorePrefix)]}
            ...
            if (likeVal.startsWith('%') && likeVal.endsWith('%')) {
                regex = `^.*?${likeVal.substring(1).substring(0, likeVal.length - 1)}$`;
            } else if (likeVal.startsWith('%')) {
                regex = `^.*?${likeVal.substring(1)}$`;
            } else if (likeVal.endsWith('%')) {
                regex = `^${likeVal.substring(0, likeVal.length - 1)}.*?$`;
            } else {
                regex = `^${likeVal}$`;
            }
     ...
     * @returns {{limit:number, collection:string, projection:object?, skip:number?, limit:number?, query:object?, sort:object? }}
     * @throws
     */
    ...
    static makeMongoAggregate(ast){
        ...
        if (ast.orderby&&ast.orderby.length>0){
            parsedQuery.pipeline.push({$sort:ast.orderby.reduce((a,v)=>{
                    a[v.expr.column||v.expr.value]=v.type==="DESC"?-1:1;
                    return a;
                },{})});
        }
        ...
        if(ast.columns&&ast.columns!=="*"&&ast.columns.length>0){
            parsedQuery.pipeline.push({$project:ast.columns.map(c=>{
                    let col={};
                    if(c.as){
