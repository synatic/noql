...
const {pipelineToSQL} = require('./toSQL');
...
class SQLParser {
    ...
    /**
     * Converts a MongoDB aggregation pipeline back into an equivalent noql
     * SQL statement. Supports a single FROM table (with alias), INNER/LEFT
     * joins (incl. |first/|last/|unwind/|optimize hints and joins against a
     * derived table, i.e. `JOIN (SELECT ...) alias`), WHERE, GROUP BY/HAVING,
     * SELECT columns (incl. aggregates/functions/CASE), ORDER BY,
     * LIMIT/OFFSET, and UNSET(...). Also reverses pipelines produced with
     * `optimizeJoins` and/or `optimizeMongoAggregate` — both only ever
     * rearrange pipeline stages for performance (predicate pushdown, join
     * reordering), never change query semantics, so this recovers the same
     * SQL as reversing the unoptimized pipeline would.
     *
     * Not supported (throws): FROM sub-selects (i.e. the outer query's own
     * FROM, as opposed to a joined derived table), UNION/INTERSECT/EXCEPT,
     * PIVOT/UNPIVOT, window functions, FULL JOIN, IN (SELECT ...), and array
     * sub-selects.
     * @param {object[]|object} pipeline - the aggregation pipeline stages, or
     *   a full aggregate command document (`{aggregate, pipeline, ...}`)
     * @param {string[]} [collections] - the base collection name(s) the
     *   pipeline runs against (optional when `pipeline` is a command document
     *   with `aggregate`)
     * @param {{pretty?: boolean}} [options] - `pretty` (default `true`)
     *   formats the SQL across multiple indented lines
     * @returns {string} the reconstructed SQL statement
     * @throws
     */
    static pipelineToSQL(pipeline, collections, options) {
        return pipelineToSQL(pipeline, collections, options);
    }
}
