...
class SQLParser {
    ...
    static _makeQueryPart(queryPart, ignorePrefix) {
        ...
        const makeOperator=(op)=>{
            const left=SQLParser._makeQueryPart(queryPart.left, ignorePrefix);
            const right=SQLParser._makeQueryPart(queryPart.right, ignorePrefix);
        ...
        if(queryPart.type==="binary_expr") {
            ...
            if (queryPart.operator === 'AND') return $merge(SQLParser._makeQueryPart(queryPart.left, ignorePrefix), SQLParser._makeQueryPart(queryPart.right, ignorePrefix));
            if(queryPart.operator === 'OR') return {$or: [SQLParser._makeQueryPart(queryPart.left, ignorePrefix), SQLParser._makeQueryPart(queryPart.right, ignorePrefix)]};
            if (queryPart.operator === 'IN') return {$in: [SQLParser._makeQueryPart(queryPart.left, ignorePrefix), SQLParser._makeQueryPart(queryPart.right, ignorePrefix)]};
