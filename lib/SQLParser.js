...
class SQLParser {
    ...
    static _makeJoinForPipeline(ast) {
        ...
        const makeJoinPart = (join,previousJoin) => {
            ...
            const toAs = join.as;
            const fromTable=previousJoin.table||'';
            ...
            if (join.table && join.on && join.on.type === 'binary_expr' && join.on.operator === '=') {
                ...
                const localField =  ((fromTable&&join.on.left.table === fromTable) || (fromAs&&join.on.left.table === fromAs))?`${localTable?localTable + '.':''}${join.on.left.column}` :
                    (join.on.right.table === fromTable || join.on.right.table === fromAs)?`${localTable?localTable + '.':''}${join.on.right.column}` :
                        `${localTable?localTable + '.':''}${join.on.right.column}`;

                const foreignField =  (join.on.left.table === toTable || join.on.left.table === toAs)?join.on.left.column :
                    (join.on.right.table === toTable || join.on.right.table === toAs)?join.on.right.column :
                        join.on.left.column;
    ...
    static _makeAggregatePipeline(ast, options= {}) {
        ...
        if (ast.groupby) {
            ...
            ast.columns.forEach((v) => {
                if (v.expr.type === 'column_ref') {
                    ...
                    unwind = [];
                    if(v.expr.args&&v.expr.args.value&&$check.array(v.expr.args.value)&&v.expr.args.value[0]&&v.expr.args.value[0].column&&v.expr.args.value[0].column!==v.as){
                        unwind.push({
        ...
        if (ast.where) {
            pipeline.unshift({$match: SQLParser._makeQueryPart(ast.where, false, [], false)});
        }
    ...
    static _validateAST(parsedAST) {
        ...
        if ($check.array(ast.columns)) {
            ...
            for (const column of ast.columns) {
                ...
                if (column.expr && ['aggr_func'].includes(column.expr.type) && !['SUM','AVG','MIN','MAX'].includes(column.expr.name) && !ast.groupby) {
    ...
    static makeMongoQuery(sqlOrAST, options = {}) {
        ...
        if (!SQLParser.canQuery(parsedAST)) {
            throw new Error('Query cannot cross multiple collections, have an aggregate function, contain functions in where clauses or have $$ROOT AS');
        }
        const ast = parsedAST.ast;
        const parsedQuery = {
            limit: 100,
