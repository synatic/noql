...
class SQLParser {
     ...
     * @private
     */
    static _makeFilterCondition=(queryPart,includeThis=false)=>{
        if(queryPart.type==="binary_expr") {
            if (queryPart.operator === '=') return {$eq:[SQLParser._makeFilterCondition(queryPart.left,includeThis),SQLParser._makeFilterCondition(queryPart.right,includeThis)]};
            if (queryPart.operator === '>') return {$gt:[SQLParser._makeFilterCondition(queryPart.left,includeThis),SQLParser._makeFilterCondition(queryPart.right,includeThis)]};
            if (queryPart.operator === '<')  return {$lt:[SQLParser._makeFilterCondition(queryPart.left,includeThis),SQLParser._makeFilterCondition(queryPart.right,includeThis)]};
            if (queryPart.operator === '>=') return {$gte:[SQLParser._makeFilterCondition(queryPart.left,includeThis),SQLParser._makeFilterCondition(queryPart.right,includeThis)]};
            if (queryPart.operator === '<=') return {$lte:[SQLParser._makeFilterCondition(queryPart.left,includeThis),SQLParser._makeFilterCondition(queryPart.right,includeThis)]};
            if (queryPart.operator === '!=') return {$ne:[SQLParser._makeFilterCondition(queryPart.left,includeThis),SQLParser._makeFilterCondition(queryPart.right,includeThis)]};
            if (queryPart.operator === 'AND') return {$and:[SQLParser._makeFilterCondition(queryPart.left,includeThis),SQLParser._makeFilterCondition(queryPart.right,includeThis)]};
            if(queryPart.operator === 'OR') return {$or:[SQLParser._makeFilterCondition(queryPart.left,includeThis),SQLParser._makeFilterCondition(queryPart.right,includeThis)]};
            if (queryPart.operator === 'LIKE') {
                let likeVal = queryPart.right.value;
                const regex=SQLParser._makeRegex(likeVal);

                return {$regexMatch:{input:SQLParser._makeFilterCondition(queryPart.left,includeThis),regex: regex, options: 'i'}};
            }
        ...
        if(queryPart.type==='column_ref')return `${includeThis?'$$this.':'$'}${queryPart.column}`;
    ...
    static _makeAggregatePipeline(ast,options) {
        ...
        if(ast.groupby){
            if(ast.columns==="*")throw new Error(`Select * not allowed with group by`);
            let groupBy={$group:{
                    _id:{}
                }
            };
        ...
        }else if (ast.columns && ast.columns !== '*' && ast.columns.length > 0) {
    ...
    static makeMongoAggregate(sqlOrAST, options = {unwindJoins: false}) {
        ...
        const getTablesToIgnore=(subAst,first)=>{
            ...
            if(subAst.columns&&subAst.columns!=="*"&&subAst.columns.length>0){
