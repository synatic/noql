...
class SQLParser {
     ...
     * joins (incl. |first/|last/|unwind hints), WHERE, GROUP BY/HAVING,
     * SELECT columns (incl. aggregates/functions/CASE), ORDER BY,
     * LIMIT/OFFSET, and UNSET(...).
     *
     * Not supported (throws): FROM sub-selects, UNION/INTERSECT/EXCEPT,
     * PIVOT/UNPIVOT, window functions, FULL JOIN, IN (SELECT ...), array
     * sub-selects, and pipelines produced with `optimizeJoins`/`unwindJoins`
     * or via `optimizeMongoAggregate`.
     ...
     * @throws
     */
    static pipelineToSQL(pipeline, collections) {
        return pipelineToSQL(pipeline, collections);
    }
}
