...
class SQLParser {
    ...
    static _makeFilterCondition=(queryPart,includeThis)=>{
    ...
    static _makeCaseCondition(expr){
        ...
        let elseExpr=expr.args.find(a=>a.type==="else");
        let whens=expr.args.filter(a=>a.type==="when");

        const makeCond=(curWhen)=>{
            let nextWhen=whens.shift();
            return {
                $cond: {
                    if: SQLParser._makeFilterCondition(curWhen.cond),
                    then:SQLParser._makeFilterCondition(curWhen.result),
                    else: nextWhen?makeCond(nextWhen): SQLParser._makeProjectionExpressionPart(elseExpr.result)
                }
            };

        };

        return makeCond(whens.shift());
    }
    ...
    static _joinFunctionLogic(on) {
        ...
        if(splitedRightColumn.length <= 1) throw new Error('Inside of join on operation table and field must present like `table.field`');
        const localField = on.left.column;
        const from = splitedRightColumn[0];
        const foreignField = splitedRightColumn[1];
        const as = from;
    ...
    static _makeAggregatePipeline(ast) {
        ...
        if (ast.columns && ast.columns !== '*' && ast.columns.length > 0) {
            const unwindColumn = ast.columns.find((column) => {
            ...
            if (unwindColumn) {
                let unwind = `$${unwindColumn.expr.args.value[0].column}`;
