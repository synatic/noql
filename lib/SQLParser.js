...
class SQLParser {
    ...
    static _makeFilterCondition=(queryPart,includeThis=false,prefixRight=false,side='left')=>{
        if(queryPart.type==="binary_expr") {
            if (queryPart.operator === '=') return {$eq:[SQLParser._makeFilterCondition(queryPart.left,includeThis,prefixRight,'left'),SQLParser._makeFilterCondition(queryPart.right,includeThis,prefixRight,'right')]};
            if (queryPart.operator === '>') return {$gt:[SQLParser._makeFilterCondition(queryPart.left,includeThis,prefixRight,'left'),SQLParser._makeFilterCondition(queryPart.right,includeThis,prefixRight,'right')]};
            if (queryPart.operator === '<')  return {$lt:[SQLParser._makeFilterCondition(queryPart.left,includeThis,prefixRight,'left'),SQLParser._makeFilterCondition(queryPart.right,includeThis,prefixRight,'right')]};
            if (queryPart.operator === '>=') return {$gte:[SQLParser._makeFilterCondition(queryPart.left,includeThis,prefixRight,'left'),SQLParser._makeFilterCondition(queryPart.right,includeThis,prefixRight,'right')]};
            if (queryPart.operator === '<=') return {$lte:[SQLParser._makeFilterCondition(queryPart.left,includeThis,prefixRight,'left'),SQLParser._makeFilterCondition(queryPart.right,includeThis,prefixRight,'right')]};
            if (queryPart.operator === '!=') return {$ne:[SQLParser._makeFilterCondition(queryPart.left,includeThis,prefixRight,'left'),SQLParser._makeFilterCondition(queryPart.right,includeThis,prefixRight,'right')]};
            if (queryPart.operator === 'AND') return {$and:[SQLParser._makeFilterCondition(queryPart.left,includeThis,prefixRight,'left'),SQLParser._makeFilterCondition(queryPart.right,includeThis,prefixRight,'right')]};
            if(queryPart.operator === 'OR') return {$or:[SQLParser._makeFilterCondition(queryPart.left,includeThis,prefixRight,'left'),SQLParser._makeFilterCondition(queryPart.right,includeThis,prefixRight,'right')]};
            if (queryPart.operator === 'LIKE') {
                let likeVal = queryPart.right.value;
                const regex=SQLParser._makeRegex(likeVal);

                return {$regexMatch:{input:SQLParser._makeFilterCondition(queryPart.left,includeThis,prefixRight,'left'),regex: regex, options: 'i'}};
            }
        ...
        if(queryPart.type==='column_ref')return `${includeThis?'$$this.':'$'}${prefixRight&&side==='right'?'$' + (queryPart.table?(queryPart.table + '.'):''):''}${queryPart.column}`;
    ...
    static _getParsedValueFromBinaryExpression(expressionPart) {
        ...
        if (expressionPart.type === 'number'||expressionPart.type==='string') return expressionPart.value;
    ...
    static _makeJoinForPipeline(ast){
        ...
        const makeJoinPart=(join)=>{
            if(join.table&&join.on&&join.on.type==="binary_expr"&&join.on.operator==='='){
                let as=join.as || join.table;
                let local=join.on.left.table===as||join.on.left.as===as?`${join.on.right.table?(join.on.right.table+'.'):''}${join.on.right.column}`:`${join.on.left.table?(join.on.left.table+'.'):''}${join.on.left.column}`;
                let foreign=join.on.left.table===as||join.on.left.as===as?join.on.left.column:join.on.right.column;
    ...
    static _makeAggregatePipeline(ast,options) {
        ...
        if(ast.groupby){
            ...
            ast.columns.forEach((v) => {
                if (v.expr.type === 'column_ref') {
                    ...
                    const aggregateFunction = _allowableFunctions.functionList.find((f) => f.name.toLowerCase() === v.expr.name.toLowerCase()&&(!f.type||f.type==="aggr_func"));
        ...
        if(!ast.from[0].table&&ast.from[0].expr&&ast.from[0].expr.ast){
            ...
            pipeline=SQLParser._makeAggregatePipeline(ast.from[0].expr.ast,options).concat([{$project: {[ast.from[0].as]:"$$ROOT"}}]).concat(pipeline);
        }
        ...
        if (ast.orderby && ast.orderby.length > 0) {
            pipeline.push({
                $sort: ast.orderby.reduce((a, v) => {
                    ...
                    let asMapped=asMapping.find(c=>c.column===v.expr.column);
        ...
        if (ast.from.length) {
            ...
            ast.from.forEach(from => {
                if(from.expr && from.expr.ast){
                    lookups.push({
                        let: {
                            [from.on.left.column.replace('.', '')]: `$${from.on.left.column.split('.')[1]}`
                        },
                        from: from.expr.ast.from[0].table,
                        pipeline: SQLParser._makeAggregatePipeline(from.expr.ast),
                        match: {
                            $expr: {
                                $eq: [`$${from.on.right.column.split('.')[1]}`, `$$${from.on.left.column.replace('.', '')}`]
                            }
                        },
                        as: from.as
                    })
                ...
                // if (from.join) {
                //     lookups.push(SQLParser._makeQueryPart(from.on));
        ...
        if (ast.columns && !SQLParser._isSelectAll(ast.columns) && ast.columns.length > 0) {
            ...
            const setIndex = ast.columns.findIndex((column) => SQLParser._isSelectAll(column.expr.column));
            if (setIndex >= 0) {
            ...
            ast.columns.forEach((column, index) => {
                if (setIndex !== index) {
                    if (column.as || column.expr.name || column.expr.column) {
                        project[column.as || column.expr.column || (column.expr && column.expr.name)] = column.expr.column || (column.expr && column.expr.name);
    ...
    static makeMongoAggregate(sqlOrAST, options = {unwindJoins: false}) {
        ...
        const getTablesToIgnore=(subAst,first)=>{
            let subSelectTables=[];

            if(subAst.type==="select"&&!first&&subAst.from){
                subSelectTables.push(subAst.from[0].table);
            }

            if(subAst.columns&&!SQLParser._isSelectAll(subAst.columns)&&subAst.columns.length>0){
                for (let column of subAst.columns){
                    if(column.expr.type==="select"){
                        if(!first)subSelectTables.push(column.expr.from[0].table);
                        subSelectTables = subSelectTables.concat(getTablesToIgnore(column.expr));
                    }else if (column.expr.type==="function"){
                        if(column.expr.args&&column.expr.args.type==="expr_list"){
                            let args=$check.array(column.expr.args.value)?column.expr.args.value:[column.expr.args.value];
                            for (let arg of args){
                                subSelectTables = subSelectTables.concat(getTablesToIgnore(arg));
                            }
                        }
                   }
                }
            }
            return subSelectTables;
        };

        let tablesToIgnore=getTablesToIgnore(ast,true);
        return {
            pipeline: SQLParser._makeAggregatePipeline(ast,options),
            collections: tableList.map((t) => t.split('::')[2]).filter(t=>!tablesToIgnore.find(i=>i===t)),
        };
    }
